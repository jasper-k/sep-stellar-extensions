...
public class JoinAndSplitTest {
    ...
    public void testJoinAndSplit() throws Exception {
        ...
        Object rebuildRes = run("REBUILD_ARGS('"+joinFieldsParam+"')", new HashMap<>());
        String rebuildArguments = (String) rebuildRes;
        Object joinRes = run("JOIN(["+rebuildArguments+"],'^')", ImmutableMap.of("ip_dst_port", "1111", "protocol", "UDP", "ip_dst_addr","185.70.112.55", "original_string","some_message"));
        ...
        Assert.assertTrue(fieldValueList.contains("185.70.112.55"));
        Assert.assertTrue(fieldValueList.size()==3);
    }
